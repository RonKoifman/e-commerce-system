...
{
	int selection, numOfOrders = customer.getNumOfOrders();
	Checkout** orders = customer.getOrders();

	if (numOfOrders == 0) {
		cout << "You have 0 orders made, make an order so you could leave a feedback." << endl;
		return;
	}
	else {
		customer.showOrders();
		cout << "Choose the order you wish to make the feedback about: "; cin >> selection;
		cout << endl;
		if (selection > numOfOrders) {
			cout << "You don't have such order." << endl;
		}
		else {
			Checkout* selectedOrder = orders[selection - 1];
			Product** products = selectedOrder->getChosenProducts();
			orders[selection - 1]->show();
			cout << "Pick the product, to write its seller the feedback: "; cin >> selection;
			cout << endl;
			if (selection > selectedOrder->getNumOfChosenProducts()) {
				cout << "You don't have such product in this order." << endl;
			}
			else {
				char text[MAX_FEEDBACK_LENGTH];
				Date date = readDate();
				getTextForFeedback(text);
				products[selection - 1]->getSeller()->addFeedback(&customer, products[selection - 1], date, text);
			}
		}
	}
}
...
Date ShopSystem::readDate()
{
	int day, month, year;
	do {
		cout << "Please enter the day: "; cin >> day; cout << endl;
		cout << "Please enter the month: "; cin >> month; cout << endl;
		cout << "Please enter the year: "; cin >> year; cout << endl;
	} while (!dateValidation(day, month, year));
...
{
	cout << "Please write your feedback here (up to 90 characters): " << endl;
	int i = 0, len = 0;
	char ch = ' ';

	// Get input from buffer
	while (i < MAX_FEEDBACK_LENGTH - 1)
	{
		ch = getchar();
		text[i++] = ch;
	...
	if (len == MAX_FEEDBACK_LENGTH - 1 || len == 0)
	{
		if (len > 0)
		{
			cout << endl << "Your feedback exceeded the 90 characters limit!." << endl;
			cleanBuffer();
		}
	}
}
