...
public:
	...
	bool cinTypeCheck();
	void usernameValidation(string& username, vector<User*> users);
	bool checkUniqueUsername(const string& username, vector<User*> users);
	...
	bool isProductNameExists(const string& productName, vector<Product*> products);
	...
	bool isProductExists(int productSerialNumber, vector<Product*> products);
	bool isProductBelongsToUser(User& user, int productSerialNumber);
	bool searchProductSelectionValidation(int& selection);
	Product* indexOfCheckoutProductValidation(unsigned int& index, vector<Product*> cart, vector<Product*> chosenProducts);
	bool isSellerExists(const User& seller, vector<User*> sellers);
	void dateValidation(int& day, int& month, int& year);
	bool areValidUsers(vector<User*> users, int& indexUser1, int& indexUser2);
};
